...
#parser.add_argument("--drop", default=0.02, type=float, help="Recurrent dropout")
...
parser.add_argument("--word_masking", default=0.4, type=float, help="Mask words with the given probability.")
...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        gru = tf.keras.layers.GRU(args.cle_dim, return_sequences=True)
        hidden = tf.keras.layers.Bidirectional(gru, "concat")(c_embeddings.to_tensor())
        hidden = tf.RaggedTensor.from_tensor(hidden, c_embeddings.row_lengths())
        gru2 = tf.keras.layers.GRU(args.cle_dim, return_sequences=False)
        c_embeddings = tf.keras.layers.Bidirectional(gru2, "concat")(hidden.to_tensor())
        ...
        layer2 = getattr(tf.keras.layers, args.rnn)(args.rnn_dim, return_sequences=True)
        hidden2 = tf.keras.layers.Bidirectional(layer2, "sum")(hidden.to_tensor())
        hidden = tf.RaggedTensor.from_tensor(hidden2, hidden.row_lengths())
        layer3 = getattr(tf.keras.layers, args.rnn)(args.rnn_dim, return_sequences=True)
        hidden3 = tf.keras.layers.Bidirectional(layer3, "sum")(hidden.to_tensor())
        hidden = tf.RaggedTensor.from_tensor(hidden3, hidden.row_lengths())
        ...
        self.compile(optimizer=tf.keras.optimizers.experimental.AdamW(jit_compile=False, weight_decay=1e-4, learning_rate=args.lr),
