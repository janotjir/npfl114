...
parser.add_argument("--evaluate", default=False, action="store_true", help="Run model on dev dataset")
parser.add_argument("--epochs", default=40, type=int, help="Number of epochs.")
parser.add_argument("--final_model", default="", type=str)
...
def scce_with_labelsmooth(y, y_hat):
    y = tf.squeeze(tf.one_hot(tf.cast(y, tf.int32), len(CIFAR10.LABELS)), axis=-2)
    return tf.keras.losses.categorical_crossentropy(y, y_hat, label_smoothing=0.1)
...
class XResNet18(tf.keras.Model):
    def __init__(self, normalize):
        ...
        x = tf.keras.layers.Lambda(lambda z: tf.keras.backend.mean(z, [1, 2]))(x)
        print(x.shape)
...
def main(args: argparse.Namespace) -> None:
    ...
    '''model_types = ['resnet9', 'resnet9', 'resnet9', 'resnet9',  'resnet9', 'resnet9', 'xresnet18', 'xresnet18']
    normalize = [True, True, True, True, True, True, True, False]
    model_paths = ["logs/resnet9_3/resnet9_ep39.h5",
    "logs/resnet9_3/resnet9_ep40.h5",
    "logs/resnet9_3/resnet9_ep36.h5",
    "logs/resnet9_5/resnet9_ep37.h5",
    ...
    "logs/resnet9_5/resnet9_ep40.h5",
    "logs/xresnet18_3/xresnet18_ep31.h5",
    "logs/cifar_competition.py-2023-03-10_174055-a=True,bs=128,d=False,e=20,m=xresnet18,s=42,t=12/xresnet18_ep20.h5"]'''


    args.logdir = 'deep_ensemble'
    ...
    if not args.test and not args.evaluate:
        ...
        train = train.map(lambda x, y: ((x, x, x, x, x, x), (y, y, y, y, y, y)))
        ...
        dev = dev.map(lambda x, y: ((x, x, x, x, x, x), (y, y, y, y, y, y)))
        ...
        #merge = tf.keras.layers.Dense(128, activation='relu')(merge)
        output = tf.keras.layers.Dense(len(CIFAR10.LABELS), activation=tf.nn.softmax)(merge)
        model = tf.keras.models.Model(inputs=ensemble_visible, outputs=output)

        model.compile(optimizer=tf.optimizers.experimental.AdamW(jit_compile=False), 
            #loss=tf.losses.SparseCategoricalCrossentropy(),
        ...
        with tf.device("/GPU:0"):
            model.fit(train, epochs=args.epochs, validation_data=dev, callbacks=[tf.keras.callbacks.LambdaCallback(on_epoch_end=save_model)])

    else:

        data = None
        if args.test:
            data = tf.data.Dataset.from_tensor_slices((cifar.test.data["images"], cifar.test.data["labels"]))
            data = data.map(image_to_float)
            data = data.map(lambda x, y: ((x, x, x, x, x, x), (y, y, y, y, y, y)))
            data = data.batch(args.batch_size)
        else:
            data = tf.data.Dataset.from_tensor_slices((cifar.dev.data["images"], cifar.dev.data["labels"]))
            data = data.map(image_to_float)
            data = data.map(lambda x, y: ((x, x, x, x, x, x), (y, y, y, y, y, y)))
            data = data.batch(args.batch_size)
        ...
        model.load_weights(args.final_model)
        ...
        if args.evaluate:
            ...
            args.logdir = "deep_ensemble_results"
