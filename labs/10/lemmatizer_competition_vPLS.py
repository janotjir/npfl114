...
class Model(tf.keras.Model):
    def __init__(self, args: argparse.Namespace, train: MorphoDataset.Dataset) -> None:
        ...
        self.w_embeddings = tf.keras.layers.Embedding(train.forms.word_mapping.vocabulary_size(), 256)
        self.c_embed_gru1 = tf.keras.layers.Bidirectional(tf.keras.layers.GRU(256, return_sequences=True), 'concat')
        self.c_embed_gru2 = tf.keras.layers.Bidirectional(tf.keras.layers.GRU(256, return_sequences=False), 'concat')
        ...
        self.tag_to_encoder = tf.keras.layers.Dense(args.emb_dim, activation=None)
        ...
        def ragged_sparse_categorical_crossentropy(y_true, y_pred):
            return tf.losses.SparseCategoricalCrossentropy(from_logits=False)(y_true.values, y_pred.values)

        # Compile the model
        self.compile(
            optimizer=tf.optimizers.Adam(learning_rate=1e-4, jit_compile=False),
            # loss={"lemmas" : tf.losses.SparseCategoricalCrossentropy(from_logits=True), "tags" : ragged_sparse_categorical_crossentropy},
            loss={"lemmas" : tf.losses.SparseCategoricalCrossentropy(from_logits=True), "tags" : tf.losses.SparseCategoricalCrossentropy()},
            metrics=[tf.metrics.Accuracy(name="accuracy")]
            #metrics={"lemmas" : tf.metrics.SparseCategoricalAccuracy(name="accuracy"), "tags" : tf.metrics.SparseCategoricalAccuracy(name="tag_accuracy")}, #tf.metrics.SparseCategoricalAccuracy(name="tag_accuracy")}],
        )

        self.tb_callback = tf.keras.callbacks.TensorBoard(args.logdir)

    def encoder(self, inputs: tf.Tensor, tags_to_encoder) -> tf.Tensor:
        # TODO(lemmatizer_noattn): Embed the inputs using `self._source_embedding`.
        embeddings = self._source_embedding(inputs)

        tag_codes = tf.reshape(tags_to_encoder, [-1, tf.shape(embeddings)[-1]])
        # tf.print(tf.shape(embeddings), tf.shape(tags_to_encoder))
        #embeddings = tf.concat([embeddings, tags_to_encoder], axis=-1)
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            #tf.print(tf.shape(pred_tags))
            #tf.print(tf.shape(pred_tags.values))
            encoded = self.encoder(x_flat, self.tag_to_encoder(pred_tags))
            pred_lemmas = self.decoder_training(encoded, y_flat)
            y_pred = {"lemmas": pred_lemmas, "tags": self.tag_final(pred_tags).values}
            y_flat = {"lemmas": y_flat.values, "tags": tags.values}
    ...
    def predict_step(self, data):
        ...
        encoded = self.encoder(data_flat, self.tag_to_encoder(pred_tags))
        ...
        # return {"lemmas" : y_pred, "tags" : tag_logits.values}
        return y_pred
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.eval:
        ...
        ep = args.model.split("/")[-1].split(".")[0][2:]
        filename = "lemmatizer_competition_val" + ep + ".txt"
    else:
